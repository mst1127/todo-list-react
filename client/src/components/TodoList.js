...
const TodoList = () => {
    ...
    const deleteHandler = async index => {
        ...
        await axios.delete(`http://localhost:3001/todolist/${deleteTodo.id}`, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Basic ${credentials.username}:${credentials.password}`
            }
    ...
    return <>
        <div className="todoContainer">
            ...
            {todos.map((todo, index) => (
                <div className={todos[index]?.checked ? 'todoItem todoItemCompleted' : 'todoItem '} key={index}>
                    {todos[index]?.checked
                    ...
                    <CloseIcon className='delTodo' onClick={() => deleteHandler(index)}/>
                </div>
            ))}
